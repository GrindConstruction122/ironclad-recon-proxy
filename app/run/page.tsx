...
export default function RunPage() {
  ...
  function handleExport() {
    if (!output) return
    const filename = `${projectName || 'grind-recon'}-${selectedTool?.id || 'output'}-${new Date().toISOString().split('T')[0]}.txt`
    ...
    <div style={{ minHeight: '100vh', background: '#000000', color: '#f0f6ff', fontFamily: 'Arial, sans-serif' }}>
      ...
      <div style={{ maxWidth: 1100, margin: '0 auto', padding: '32px 24px' }}>
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, justifyContent: 'center', marginTop: 12 }} onClick={e => e.stopPropagation()}>
              {pendingFiles.map((f, i) => (
                <div key={i} style={{
                  ...
                  <button onClick={() => removeFile(i)} style={{ background: 'none', border: 'none', color: '#7F9DB1', cursor: 'pointer', fontSize: '1rem', lineHeight: 1, padding: 0 }}>×</button>
                </div>
              ))}
            </div>
          )}
        ...
        <div className="no-print" style={{ marginBottom: 20, display: 'flex', alignItems: 'center', gap: 16 }}>
          <span style={{ fontSize: '0.7rem', fontWeight: 700, letterSpacing: 2, textTransform: 'uppercase', color: '#7F9DB1', flexShrink: 0 }}>Project Name</span>
          <input
            type="text"
            value={projectName}
          ...
          {activeCat.tools.map(tool => (
            ...
            >
              <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 6 }}>
                <h3 style={{ fontWeight: 700, fontSize: '0.9rem', color: selectedTool?.id === tool.id ? '#C3E3EB' : '#f0f6ff', lineHeight: 1.3, margin: 0 }}>{tool.name}</h3>
                <span style={{
                  fontSize: '0.55rem',
                  fontWeight: 700,
                  letterSpacing: 1,
              ...
              <p style={{ fontSize: '0.75rem', color: '#7F9DB1', lineHeight: 1.5, marginBottom: 0, marginTop: 0 }}>{tool.desc}</p>
            </div>
          ))}
        </div>
        ...
        {selectedTool && (status === 'idle' || status === 'error') && (
          <div className="no-print" style={{ background: '#0d0d0d', border: '1px solid #003B66', borderRadius: 6, padding: 20, marginBottom: 24 }}>
            ...
            <div style={{ fontSize: '0.7rem', fontWeight: 700, letterSpacing: 2, textTransform: 'uppercase', color: '#7F9DB1', marginBottom: 6 }}>Additional Context (Optional)</div>
            <textarea
              value={context}
  ...
  return (
    <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.85)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, padding: 20 }}>
      <div style={{ background: '#0d0d0d', border: '1px solid #003B66', borderRadius: 6, padding: 28, maxWidth: 520, width: '100%', boxShadow: '0 20px 60px rgba(0,0,0,0.8)' }}>
        ...
        {!submitted ? (
          <>
            <div style={{ fontSize: '0.75rem', color: '#7F9DB1', marginBottom: 8, fontWeight: 700, letterSpacing: 2, textTransform: 'uppercase' }}>
            ...
            <div style={{ display: 'flex', gap: 8 }}>
              ...
              <button
                ...
                disabled={submitting}
                style={{ background: '#3D4EAC', border: 'none', color: '#fff', fontWeight: 900, fontSize: '0.7rem', letterSpacing: 2, textTransform: 'uppercase', padding: '10px 18px', borderRadius: 4, cursor: submitting ? 'not-allowed' : 'pointer', opacity: submitting ? 0.6 : 1 }}
              >
